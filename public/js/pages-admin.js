...
App.page('settings', {
  ...
  help: {
    ...
    terms: [
      ...
      ['公司證照', '室內裝修業登記證與專業技術人員登記證有效期 5 年，逾期不得從事室內裝修。填了到期日，系統會提前開待辦。'],
  ...
  async render(el) {
    ...
    el.innerHTML = `<div class="card"><div class="form-grid" id="st">
      ...
      ${row('review_days', '契約審閱期（天）', '範本至少 7 天；合約的「交付業主審閱日」到簽約日少於這個天數會提醒')}
      ${row('client_bond_pct', '交給業主的保固保證金（合約總價％）', '範本不得低於 5%；驗收交屋後記錄不足會提醒')}
      ${row('acceptance_days', '業主會同驗收期限（天）', '範本為收到完工書面通知翌日起 10 日內')}
      <h4 class="form-sec">公司證照</h4>
      ${row('license_no', '室內裝修業登記證字號')}
